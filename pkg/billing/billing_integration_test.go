...
func TestRealTimeBilling(t *testing.T) {
	...
	t.Logf("Using temporary database: %s", dbFile)
	...
	t.Run("paying account accrues and charges minutes over real time", func(t *testing.T) {
		...
		initialMinutes := 1000
		createTestAccount(t, testDB, uid, "N", initialMinutes)
	...
	t.Run("complimentary account never accrues charges even over minutes", func(t *testing.T) {
		...
		initialMinutes := 1000
		createTestAccount(t, testDB, uid, "Y", initialMinutes)
		...
		finalMinutes := getAccountMinutes(t, testDB, uid)
